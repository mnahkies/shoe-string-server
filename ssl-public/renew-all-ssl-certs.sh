...
docker run -i --rm --user="1000:1000" \
  ...
  -v="${DATA_BASE_PATH}:/home/node" node:24-alpine node --experimental-strip-types /code/ssl-public/generate-renew-all-ssl-certs.ts /home/node/applications-public | while read -r line; do
  "${__dir}"/$line
done
